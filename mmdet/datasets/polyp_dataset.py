...
class PolypDataset(CustomDataset):
    ...
    def __init__(self, clean_ann=False,empty_ann=1, **kwargs):
        ...
        super(PolypDataset, self).__init__(**kwargs)   
    ...
    def load_annotations(self, ann_file):
        ...
        if "train" in os.path.basename(ann_file):
            self.split = "train"
        elif "test" in os.path.basename(ann_file):
            self.split = "test"
        elif "val" in os.path.basename(ann_file):
            self.split = "test"
        base_dir = self.data_root
        ann_file_path = os.path.join(self.data_root,self.split+'_annotation.json')
        #if self.clean_ann:
        #    ann_file_path = os.path.join(self.data_root,self.split+'_annotation_clean.json')
        if not os.path.exists(ann_file_path):
            anno_files = glob.glob(os.path.join(base_dir, "annos/{}".format(self.split), '*.json'))
            assert len(anno_files) > 0, 'No annotation files locat at: {}'.format(
                os.path.join(base_dir, "annos/{}".format(self.split)))
            ...
            print('{} set contains {} images'.format(self.split, len(self.image_paths)))
            ...
            for i, file_name in enumerate(self.image_paths):
                ...
                for j in range(len(gt_bboxs)):
                    if gt_bboxs[j][4]==2:
                        print("instrument")
                    if gt_bboxs[j][4]==1:
                        bboxes.append(gt_bboxs[j][:4].tolist())
                        labels.append(gt_bboxs[j][4].tolist())
            ...
            #json_data_infos = {'data_infos':data_infos}

            mmcv.dump(data_infos,ann_file_path)

        else:
            print("annotation exists!")
            data_infos = mmcv.load(ann_file_path)
            #data_infos = json_data_infos['data_infos']
        # print('\n', '*' * 80, '\n', 'data_infos in total: ', data_infos)
        self.img_ids=[i for i in range(len(data_infos))]
        ...
        if self.clean_ann:
            return self.clean_empty_ann(data_infos)
        if self.empty_ann>1:
            return self.generate_more_empty(data_infos)
        return data_infos
    def generate_more_empty(self,data_infos):
        tempty_img_infos = []
        empty_img_infos = []
        for img_info in data_infos:
            if len(img_info['ann']['bboxes'])==0:
                empty_img_infos.append(img_info)
        for i in range(self.empty_ann-1):
            tempty_img_infos+=empty_img_infos

        data_infos = data_infos+tempty_img_infos
        import random
        random.shuffle(data_infos) 
        return data_infos        

    def clean_empty_ann(self,data_infos):

        for img_info in data_infos:
            if len(img_info['ann']['bboxes'])==0:
                data_infos.remove(img_info)
        return data_infos
